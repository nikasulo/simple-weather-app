...
const WeatherCardList = ({ currentStartIndex, upperLimitOfCards, handleClick, selectedIndex }: Props) => {
    ...
    <Box display="flex" flexDirection="row" style={{justifyContent: 'space-around'}}>
        {
          weatherData.map((data: Weather, index: number) => {
            ...
            return (
              <WeatherCard selected={index === selectedIndex} onClick={() => handleClick(index)} data={data}/>
            )
          })
        }
      </Box>
